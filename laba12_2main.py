...
class BankAccount:
    def get_info(self):
        ...
        return login, deposit, dep_type

    def __init__(self):
        self.login = self.get_info()[0]
        self.deposit = int(self.get_info()[1])
        self.type = self.get_info()[2]
...
while key != 5:
    ...
    if key == 4:
        print('\n'*10)
        print(f'Номер счета: {user.get_info()[0]}\n'
              f'Тип счета: {user.get_info()[2]}')
    if 0 < key > 5:
        print('\n'*10)
